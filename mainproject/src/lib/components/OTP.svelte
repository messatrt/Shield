<script>
  ...
  function handleInput(index, event) {
    ...
    if (/^\d$/.test(value)) {
      ...
      if (index === 5 && otpValues.every(val => val !== '')) {
        verifyOtp();
      }
    } else {
      otpValues[index] = '';
  ...
  function verifyOtp() {
    ...
    // Call the onVerify function passed from parent
  ...
  function resendOtp() {
    ...
    // Notify parent component to resend OTP
    dispatch('resend');
  }
</script>
...
<div class="resend-container">
  ...
  {:else}
    <button on:click={resendOtp} class="resend-button" disabled={isSubmitting}>Resend Code</button>
  {/if}
</div>
